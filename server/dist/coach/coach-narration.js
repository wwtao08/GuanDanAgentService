...
export function spokenRank(rank) {
    switch (rank) {
        case '10':
            return '十';
        case 'J':
 ...
 */
export function describePlayColloquially(pattern) {
    if (!pattern) {
        return '（无牌）';
    }
 ...
 */
export function describeSeatPlayLine(seatLabel, pattern) {
    return `${seatLabel}${describePlayColloquially(pattern)}`;
}
/**
 * 你要压过的那一手（带方位），用于提示「上一手」口语块
 */
export function describePressureFromLastPlay(myPosition, lastPlayerIndex, pattern) {
