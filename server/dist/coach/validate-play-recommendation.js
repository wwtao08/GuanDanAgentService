 ...
 */
export function validateLlmPlayRecommendation(action, cardIds, handCards, lastPlayedPattern, judgeCtx, lastPlayerIndex) {
    ...
    for (const id of cardIds) {
        if (seen.has(id))
            return { ok: false, reason: '重复的牌 id' };
        seen.add(id);
        const c = handById.get(id);
        if (!c)
            return { ok: false, reason: '含有不在手牌中的 id' };
        picked.push(c);
    }
    if (picked.length === 0) {
...
export function toCoachRecommendedFromCards(cards, judgeCtx) {
