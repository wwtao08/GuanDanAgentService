...
import { buildCoachSystemPrompt, buildCoachUserPrompt, buildCoachPlayRecommendationSystemPrompt, buildCoachPlayRecommendationUserPrompt, } from './prompt-builder.js';
...
function safeJsonExtract(text) {
    ...
    try {
        ...
        if (direct && typeof direct === 'object' && 'reason' in direct)
            return direct;
    }
    catch {
        /* fall through */
...
function looksLikeChineseCoachReason(s) {
    const t = s.trim();
...
function isKimiK2FamilyModel(model) {
    return model.startsWith('kimi-k2');
}
/** Kimi K2 系列（如 kimi-k2.5）仅允许 temperature=0.6，否则会 400 */
function setCoachRequestTemperature(body, model, defaultNonK2) {
...
function maxTokensForMode(coachMode) {
    return coachMode === 'expert' ? 240 : 520;
}
async function callOpenAIChat(systemPrompt, userPrompt, timeoutMs, maxTokens, outputFormat) {
    const key = resolveOpenAiApiKey();
    if (!key)
        throw new Error('OpenAI API key missing (set environment variable OPENAI_API_KEY)');
...
async function callOpenAIChatStreamResponse(systemPrompt, userPrompt, timeoutMs, maxTokens) {
    const key = resolveOpenAiApiKey();
    if (!key)
        throw new Error('OpenAI API key missing (set environment variable OPENAI_API_KEY)');
    const ctrl = new AbortController();
...
function safeParsePlayRecommendationJson(text) {
    ...
    const tryParse = (s) => {
        try {
            ...
            const cardIds = Array.isArray(o.cardIds)
                ? o.cardIds.filter((x) => typeof x === 'string')
                : [];
...
async function callOpenAIPlayRecommendation(systemPrompt, userPrompt, timeoutMs) {
    const key = resolveOpenAiApiKey();
    if (!key)
        throw new Error('OpenAI API key missing (set environment variable OPENAI_API_KEY)');
    const ctrl = new AbortController();
...
export class ReasonEngine {
     ...
     */
    async invokeLlmNonStream(params) {
        ...
        try {
            ...
            if (isAbortError(e)) {
                console.warn(`[coach] LLM non-stream timed out after ${timeoutMs}ms (AbortError). Check network/API or COACH_REASON_TIMEOUT_MS.`);
            }
            else {
    ...
    async generateReasonStream(params, onDelta) {
        const timeoutMs = gameConfig.server.coachReasonTimeoutMs;
        ...
        try {
            ...
            if (isAbortError(e)) {
                console.warn(`[coach] LLM stream timed out after ${timeoutMs}ms (AbortError), will try non-stream then fallback.`);
            }
            else {
     ...
     */
    async fetchLlmValidatedPlay(input, judgeCtx) {
        if (!gameConfig.server.coachUseLlm || !resolveOpenAiApiKey())
            return null;
        const timeoutMs = gameConfig.server.coachReasonTimeoutMs;
        ...
        try {
            ...
            const v = validateLlmPlayRecommendation(parsed.action, parsed.cardIds, input.handCards, input.lastPlayedPattern, judgeCtx, input.context.lastPlayerIndex);
            if (!v.ok) {
     ...
     */
    async fetchLlmPlayRecommendation(input, judgeCtx) {
     ...
     */
    async callLLMForMessage(prompt) {
        if (!gameConfig.server.coachUseLlm || !resolveOpenAiApiKey())
            return null;
