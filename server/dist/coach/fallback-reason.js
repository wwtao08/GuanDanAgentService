...
import { buildPartnerAndThreatLine, buildRoundFinishLine, relSeatLabel } from './hand-context.js';
/** 口语化一手牌（「出了一对红桃十…」）；供外部或兜底文案复用 */
export function describeFacingPattern(pattern) {
    if (!pattern)
        return '首轮可任意合法领出';
    return describePlayColloquially(pattern);
}
function describeLastPlayShort(my, lastPlayerIndex, pattern) {
    if (!pattern || lastPlayerIndex < 0)
        return '本轮由你领出';
    return `${relSeatLabel(my, lastPlayerIndex)}${describePlayColloquially(pattern)}`;
}
function hasHighValueCards(cards) {
    return cards.some((c) => c.value >= 15);
}
/** 与局面绑定的伪随机种子，同局面稳定、不同局面可区分 */
function coachSeed(handCards, context, action) {
    let h = 0;
...
export function buildCoachReferenceNotes(params) {
    ...
    if (lowEnemies.length) {
        lines.push(`· 终局信号：${lowEnemies.map((s) => `${relSeatLabel(my, s.position)}剩 ${s.remainingCards} 张`).join('、')}，控制与送牌权重上升。`);
    }
 ...
 */
export function buildFallbackReason(params) {
    const { recommended, lastPlayedPattern, handCards, context } = params;
