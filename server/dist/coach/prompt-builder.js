...
import { describePressureFromLastPlay, describeSeatPlayLine, humanCardName, } from './coach-narration.js';
import { buildCoachVarietyBlock, buildProverbMandateBlock } from './coach-knowledge-base.js';
import { buildHandStructureSummary, buildHandValueTopLine, buildPartnerAndThreatLine, buildRoundFinishLine, buildSeatsLine, buildTrickLine, relSeatLabel, } from './hand-context.js';
import { GUANDAN_PLAY_RULES_BLOCK, GUANDAN_TERMINOLOGY_BLOCK } from './guandan-rules-prompt.js';
function historyLinesWithSeats(playedHistory, myPosition) {
...
function fullHistoryLinesWithSeats(playedHistory, myPosition) {
    if (!playedHistory.length)
        return '（本副尚未有人出牌）';
...
function lastPlayedPatternDetail(pattern, lastPlayerIndex, myPosition) {
...
export function buildCoachUserPrompt(params) {
    ...
    const rec = recommended.action === 'pass'
        ? '【本步推荐】不出（过）'
...
export function buildCoachPlayRecommendationSystemPrompt() {
    return [
