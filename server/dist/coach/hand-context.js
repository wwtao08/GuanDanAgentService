 ...
 */
export function buildHandStructureSummary(handCards, levelRank) {
 ...
 */
export function buildSeatsLine(seats, myPosition) {
    return seats
        .map((s) => {
...
export function buildTrickLine(params) {
    const rel = (p) => (p === params.myPosition ? '我' : `座位${p}`);
    const last = params.lastPlayerIndex < 0 ? '本轮尚无人出牌（或你领出）' : `当前圈最大牌在 ${rel(params.lastPlayerIndex)}`;
    const lead = `本圈领出：座位 ${params.firstPlayerIndex}；当前轮到：${rel(params.currentPlayerIndex)}`;
    return `${lead}。${last}。`;
}
/** 相对本家的方位：上家 / 下家 / 对家（队友） / 本家 */
export function relSeatLabel(myPosition, seatPosition) {
 ...
 */
export function buildPartnerAndThreatLine(seats, myPosition) {
...
/** 本副已确定名次（掼蛋：头游～末游） */
export function buildRoundFinishLine(roundFinishOrder, myPosition) {
    if (!roundFinishOrder.length)
        return '本副尚未有人出完牌（名次未定）';
 ...
 */
export function buildHandValueTopLine(handCards, topN = 5) {
    if (!handCards.length)
        return '';
    const vc = valueCounts(handCards);
    const rankLabel = (v) => {
