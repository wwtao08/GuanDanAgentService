export const COACH_GUANDAN_PROVERBS = [
 ...
 */
export function formatProverbsForPrompt(entries = COACH_GUANDAN_PROVERBS) {
    return entries
        .map((e, i) => {
...
export function buildProverbMandateBlock(params) {
    ...
    const pushLine = (line) => {
        ...
        if (e && !chosen.some((x) => x.line === line))
            chosen.push(e);
    };
    const enemy4 = context.seats.some((s) => s.position !== my && s.position !== partner && s.remainingCards === 4);
    if (enemy4) {
        pushLine('枪不打四');
        pushLine('枪打四，不炸也要防');
    }
...
export function buildCoachVarietyBlock(varietySeed) {
    const angles = [
...
export function buildCoachKnowledgePrompt(context) {
    const my = context.myPosition;
    const partner = (my + 2) % 4;
    const lines = [];
    const enemy4 = context.seats.some((s) => s.position !== my && s.position !== partner && s.remainingCards === 4);
    ...
    const low = context.seats.filter((s) => s.position !== my && s.position !== partner && s.remainingCards > 0 && s.remainingCards <= 6);
    if (low.length && !enemy4) {
        lines.push(`【与局面可能相关】敌方有人低张（≤6）：顺口溜里「残局」「送牌」「顶牌」类条目可多对照，仍以实际手牌为准。`);
    }
    if (lines.length) {
        lines.push('');
    }
