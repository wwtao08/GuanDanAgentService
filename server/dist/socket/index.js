...
function buildHintInputForAi(room, roomId, playerId) {
...
export function setupSocket(io) {
    ...
    socketServer.on('connection', (socket) => {
        ...
        socket.on('request-coach-hint', async (payload, callback) => {
            ...
            const coachMode = rawMode === 'expert' ? 'expert' : 'beginner';
            ...
            const respondError = (errorCode, errorMessage) => {
            ...
            try {
                ...
                const result = await service.buildCoachHintStream(hintInput, {
                    ...
                    onReasonDelta: (text) => {
                        if (!text)
                            return;
                        const chunkPayload = { roomId, requestId: reqId, text };
...
async function handleAITurn(roomId, io) {
    const room = rooms.get(roomId);
    if (!room)
        return;
    const state = room.game.getState();
    if (state.status === 'game_over' || state.status === 'waiting')
        return;
    if (state.status === 'tribute') {
        const runTribute = () => {
...
async function executeAiPlayTurn(room, roomId, io) {
    ...
    const emitCardsPlayed = (cards, result) => {
