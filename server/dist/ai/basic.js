import { analyzePattern } from '../game/judge.js';
export class BasicAI {
    constructor(game, difficulty = 'normal') {
        this.game = game;
        this.difficulty = difficulty;
    }
    selectCards(cards, lastPattern) {
        if (lastPattern === null) {
    ...
    selectBestBeating(beatingCards, target) {
        if (this.difficulty === 'easy') {
        ...
        return beatingCards.reduce((min, curr) => curr.length < min.length ? curr : min);
    }
    shouldPass(cards, lastPattern) {
        if (this.difficulty === 'easy') {
    ...
    findSmallestBeatable(cards, target) {
