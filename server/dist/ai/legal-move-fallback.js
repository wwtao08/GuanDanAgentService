 ...
 */
export function findLegalBeatingPlay(hand, target, judgeCtx) {
    if (hand.length === 0)
        return null;
    let tried = 0;
    let best = null;
...
export function findMinimalLegalLead(hand, judgeCtx) {
    if (hand.length === 0)
        return null;
    const sorted = [...hand].sort((a, b) => a.value - b.value);
    for (const c of sorted) {
        if (analyzePattern([c], judgeCtx))
            return [c];
    }
    return null;
}
