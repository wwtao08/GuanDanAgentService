 ...
 */
export function buildResolvedServerConfig(defaults = serverRuntimeDefaults) {
    const openAiApiBase = process.env.OPENAI_API_BASE?.trim() || defaults.defaultOpenAiApiBase.replace(/\/$/, '');
    const openAiApiKey = process.env.OPENAI_API_KEY?.trim() || '';
    const coachLlmModel = process.env.COACH_OPENAI_MODEL?.trim() || defaults.coachDefaultOpenAiModel;
    const coachReasonTimeoutMs = parsePositiveInt(process.env.COACH_REASON_TIMEOUT_MS, defaults.coachReasonTimeoutMs);
    return {
